...
namespace {
	...
	MStatus writeAnimatedPlugs(std::ofstream& animFile, const MPlugArray& animatedPlugs, const MString& nodeName, unsigned int depth, unsigned int childCount) {
		...
		unsigned int numPlugs = animatedPlugs.length();
		animFile << "    {\n";
		animFile << "        joint = \""<<nodeName.asChar()<<"\",\n";
		animFile << "        pipeline = {\n";
		for (unsigned int i = 0; i < numPlugs; i++) {
			...
			if (strcmp(fnLeafAttr.name().asChar(), "inverseScale") == 0|| strcmp(fnLeafAttr.name().asChar(), "scaleX") == 0|| strcmp(fnLeafAttr.name().asChar(), "scaleY") == 0|| strcmp(fnLeafAttr.name().asChar(), "scaleZ") == 0) {
				continue;
			}
			animFile << "            {\n";
