...
namespace Rain {
	namespace Collision
	{
		...
		void GetSize(Math::Vector3 scale, const int vertexCount, const Render::StaticVertexFormat* vertexData, float& outX, float& outY, float& outZ);
		void GetSize(Math::Vector3 scale, const int vertexCount, const Render::StaticVertexFormat* vertexData, float& outRadius);
	};
}
